...
try {
  const maybeUndici = (
    globalThis as unknown as {
      fetch?: {
        __next_internal_get_undici_agent?: {
          setGlobalDispatcher?: unknown;
          Agent?: unknown;
        };
      };
    }
...
/**
 * Ensure we have at least two weeks for delta comparison.
 * When a single week is selected, include the immediately previous
 * available week (older) only for delta calculations. This MUST NOT
...
async function weeksForDelta(selectedWeeks?: string[]): Promise<string[] | undefined> {
  ...
  const all = await getAvailableWeeks(); // Sorted desc (newest -> oldest)
  const idx = all.indexOf(target);
  if (idx >= 0 && idx + 1 < all.length) {
    const prev = all[idx + 1] as string;
    // Return union (target + previous). Order does not matter; splitWeeksSets sorts.
    return Array.from(new Set([target, prev])) as string[];
  }
...
function createPublicClient(): SupabaseClient<Database> {
  ...
  return createSbClient<Database>(url as string, anonKey as string, {
    ...
    global: {
      headers: { "x-app": "dashboard-inbound" },
...
export async function getClusterStats(runId: string) {
  ...
  const { count: clusterCount } = await supabase
    .from("blog_clusters")
    .select("cluster_id", { count: "planned", head: true })
    .eq("run_id", runId)
    .gte("cluster_id", 0);
...
export async function getClusterLeaderboard(runId: string, selectedWeeks?: string[]) {
  ...
  return unstable_cache(
    async () => {
      ...
      // Base period metrics: strictly from selected weeks
      const articleMetricsBase = await fetchArticleMetrics(selectedWeeks);
      ...
      // Aggregate base period totals
      for (const m of articleMetricsBase) {
      ...
      // Delta period: may include previous week when only one week selected
      const deltaSets = splitWeeksSets(deltaWeeks);
      const articleMetricsForDelta = await fetchArticleMetrics(deltaWeeks);
      for (const m of articleMetricsForDelta) {
        const url = m.url as string | undefined;
        if (!url) continue;
        const cid = urlToClusterId.get(url);
        ...
        } else if (w && deltaSets.late.has(w)) {
...
export async function getClusterUrlsMetrics(
  ...
  return unstable_cache(
    async () => {
      ...
      // Base period data (only selected weeks) – used for main totals
      const allData = await fetchArticleMetrics(selectedWeeks, urls);
      ...
      >((acc, item) => {
        ...
        const uExisting = acc[key];
        if (!uExisting) return acc;
        const u = uExisting;
      ...
      // Compute deltas per URL using early vs late buckets over delta dataset
      const earlyAgg: Record<string, Accumulator> = {};
      const lateAgg: Record<string, Accumulator> = {};
      for (const item of allDataForDelta) {
        const key = item.url as string;
        if (!key) continue;
        ...
        if (isEarly) {
          ...
          target = earlyAgg[key] ?? null;
        } else if (isLate) {
          if (!lateAgg[key]) lateAgg[key] = createAccum();
          target = lateAgg[key] ?? null;
        }
        if (!target) continue;
      ...
      let rows: ClusterUrlAggregates[] = Object.values(perUrl).map((u) => {
        ...
        return {
          ...
          gsc_clicks_delta_pct:
            e.clicks > 0
              ? (l.clicks - e.clicks) / e.clicks
              : l.clicks - e.clicks > 0
                ? Number.POSITIVE_INFINITY
                : 0,
    ...
    [
      ...
      String(clusterId),
      weeksKeyOf(selectedWeeks),
      `delta:${weeksKeyOf(deltaWeeksForKey)}`,
      `l:${limit}`,
      `o:${offset}`,
      version,
      `rv:${runsVersion}`,
